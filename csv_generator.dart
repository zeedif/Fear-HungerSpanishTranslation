...
class JsonProcessor {
  ...
  JsonProcessor(this.translationCode, this.generateTranslationsFile)
      : originDirectory = 'data',
        translationsDirectory = 'csv/$translationCode',
  ...
  Future<void> processFile(String fileName) async {
    if (fileName.isEmpty) {
      ...
      if (generateTranslationsFile) {
        translationsFile = File('$translationsDirectory/${fileName.replaceAll('.json', '.csv')}');
        ...
        if (!isExistsTranslationFile) {
          ...
          if (existingTranslations.isNotEmpty) {
            final rows = const CsvToListConverter().convert(existingTranslations);
            for (var row in rows) {
              var key = row[0];
      ...
      if (rows.isNotEmpty) {
        ...
        await translationsFile.writeAsString(csv);
      }
